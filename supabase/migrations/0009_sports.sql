-- =============================================================================
-- 0009_sports.sql — broaden activities beyond football into multiple sports
--
-- The data model always had `activity_type` (0001); this lights up more of it.
-- We add the popular sports as enum values and let discovery return every
-- sport (the app filters by the chosen sport chip), instead of hard-coding
-- football. Football keeps its format/skill fields; other sports ignore them.
--
-- NOTE: `alter type ... add value` cannot be used in the *same* transaction
-- that then *uses* the new value. We only add values + recreate a function
-- whose signature references the enum type (not its literals), so this is safe.
-- If your client wraps this file in one transaction and errors, run the
-- `alter type` lines on their own first, then the rest.
-- =============================================================================

alter type activity_type add value if not exists 'running';
alter type activity_type add value if not exists 'cycling';
alter type activity_type add value if not exists 'tennis';
alter type activity_type add value if not exists 'padel';
alter type activity_type add value if not exists 'basketball';
-- 'gym' and 'networking' already exist from 0001.

-- Discovery now returns the activity's sport and no longer restricts to
-- football. `type_filter` stays available (default null = every sport) so the
-- RPC can still be narrowed server-side later; today the app filters by chip.
drop function if exists public.nearby_activities(
  double precision, double precision, double precision, activity_type
);

create or replace function public.nearby_activities(
  lat double precision,
  lng double precision,
  radius_meters double precision default 25000,
  type_filter activity_type default null
)
returns table (
  id               uuid,
  host_id          uuid,
  host_name        text,
  title            text,
  venue_label      text,
  venue_lat        double precision,
  venue_lng        double precision,
  starts_at        timestamptz,
  duration_minutes integer,
  max_players      integer,
  status           activity_status,
  activity_type    activity_type,
  format           text,
  skill_level      text,
  distance_meters  double precision,
  joined_count     integer
)
language sql
stable
set search_path = public
as $$
  select
    a.id,
    a.host_id,
    p.display_name as host_name,
    a.title,
    a.venue_label,
    st_y(a.location::geometry) as venue_lat,
    st_x(a.location::geometry) as venue_lng,
    a.starts_at,
    a.duration_minutes,
    a.max_players,
    a.status,
    a.activity_type,
    a.format,
    a.skill_level,
    st_distance(a.location, st_setsrid(st_makepoint(lng, lat), 4326)::geography) as distance_meters,
    participant_count(a.id) as joined_count
  from public.activities a
  join public.profiles p on p.id = a.host_id
  where (type_filter is null or a.activity_type = type_filter)
    and a.status in ('open', 'full')
    and a.starts_at > now()
    and st_dwithin(a.location, st_setsrid(st_makepoint(lng, lat), 4326)::geography, radius_meters)
    and not public.is_blocked(auth.uid(), a.host_id)
  order by distance_meters asc, a.starts_at asc;
$$;
