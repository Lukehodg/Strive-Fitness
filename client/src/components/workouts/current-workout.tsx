...
const CurrentWorkout: React.FC<CurrentWorkoutProps> = ({ 
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-4">
      <h3 className="font-['Inter',sans-serif] text-lg font-semibold mb-4">Current Workout</h3>
      <div className="bg-primary/5 border border-primary/20 rounded-lg p-4">
        <div className="flex justify-between items-center mb-3">
          <h4 className="font-medium text-primary">{workoutName}</h4>
          <span className="bg-primary/10 text-primary text-xs font-medium px-2 py-1 rounded">In Progress</span>
        </div>
        
        <div className="space-y-3 mb-4">
          {exercises.map((exercise) => (
            <div key={exercise.id} className="flex justify-between items-center pb-2 border-b border-gray-200">
              <div>
              ...
              <div className="flex items-center">
                {exercise.isCompleted ? (
                  ...
                  <CheckCircleOutlineIcon className="text-gray-300 w-5 h-5" />
                )}
              </div>
            </div>
          ))}
